...
public class JavaTypeResolverDefaultImpl implements JavaTypeResolver {
    ...
    public JavaTypeResolverDefaultImpl() {
        ...
        typeMap.put(Types.FLOAT, new JdbcTypeInformation("FLOAT", //$NON-NLS-1$
                new FullyQualifiedJavaType(Double.class.getName())));
        //修改理由：mysql数据库设计时是使用int(11)，带符号位表示和java一样。无符号位则比Integer.MAX_VALUE要大一倍，使用Integer就存不下了，所以改成Long
        //modify by liuzhsh,原来是Byte，改为Integer
        ...
        typeMap.put(Types.NULL, new JdbcTypeInformation("NULL", //$NON-NLS-1$
                new FullyQualifiedJavaType(Object.class.getName())));
        //修改理由：java中BigDecimal不常用，java中Double的长度已经够用
        //modify by liuzhsh,原来是BigDecimal，改为Double
        ...
        typeMap.put(Types.REF, new JdbcTypeInformation("REF", //$NON-NLS-1$
                new FullyQualifiedJavaType(Object.class.getName())));
        //修改理由：java中Short不常用
        //modify by liuzhsh,原来是Short，改为Integer
    ...
    /**
     * comments add by liuzhsh,根据jdbctype转换成javatype
     */
    ...
    protected FullyQualifiedJavaType calculateBigDecimalReplacement(IntrospectedColumn column, FullyQualifiedJavaType defaultType) {
        ...
        if (column.getScale() > 0 || column.getLength() > 18 || forceBigDecimals) {
        	...
        	//add by liuzhsh,20170301，修改成NUMERIC字段小于4的使用Integer
        	answer = new FullyQualifiedJavaType(Integer.class.getName());
